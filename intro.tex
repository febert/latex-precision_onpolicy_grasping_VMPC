...
\caption{\small{Closed-loop Visual MPC: Large amounts of autonomously collected training data are used to train a image-to-image registration network and a video prediction model. At test time the current image $I_t$ is registred to both the start $I_0$ and goal-image $I_g$. \todo{add feedback arrow!}}}
\label{fig:overview}
\vspace{-0.4in}
\end{wrapfigure}
...
The main contribution of this work is a method for computing the planning cost based on image-to-image registration by using a learned registration model to find correspondences  between the current image and both the goal image and the initial image. See \autoref{fig:overview} for an illustration.
